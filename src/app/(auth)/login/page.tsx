...
export default function LoginPage() {
  ...
  return (
    <div>
      ...
      <form action={action} style={{ display: "flex", flexDirection: "column", gap: "18px" }}>
        ...
        <div>
          ...
          {state?.success === false && state.errors?.email && (
            <p style={{ color: "#F87171", fontSize: "13px", marginTop: "5px" }}>{state.errors.email[0]}</p>
          )}
        </div>
        ...
        <div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "7px" }}>
            ...
            <Link href="/recuperar-senha" style={{ color: "#10B981", fontSize: "14px", textDecoration: "none" }}>
              Esqueci a senha
            </Link>
          </div>
          <div style={{ position: "relative" }}>
            <input name="password" type={showPw ? "text" : "password"} placeholder="••••••••" autoComplete="current-password" required style={{ ...INPUT, paddingRight: "44px" }} />
          ...
          {state?.success === false && state.errors?.password && (
            <p style={{ color: "#F87171", fontSize: "13px", marginTop: "5px" }}>{state.errors.password[0]}</p>
          )}
        </div>
      ...
      <p style={{ color: "#444", fontSize: "14px", textAlign: "center", marginTop: "26px" }}>
        Não tem conta?{" "}
